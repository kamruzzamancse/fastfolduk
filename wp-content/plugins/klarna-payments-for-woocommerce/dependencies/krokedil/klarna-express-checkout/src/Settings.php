...
class Settings
{
    ...
    public function get_settings()
    {
        ...
        return array('kec_enabled' => $settings['kec_enabled'] ?? 'no', 'kec_credentials_secret' => $settings['kec_credentials_secret'] ?? '', 'kec_theme' => $settings['kec_theme'] ?? 'dark', 'kec_shape' => $settings['kec_shape'] ?? 'default', 'kec_placement' => $settings['kec_placement'] ?? 'both', 'kec_flow' => $settings['kec_flow'] ?? 'two_step', 'kec_webhook' => get_option('kec_webhook', array()), 'kec_signing_key' => get_option('kec_signing_key', array()));
    }
    /**
    ...
    public static function webhook_button($section)
    {
        ...
        if ($section['webhook_created']) {
            ...
            ?>
			<tr class="kp_settings__text_info <?php 
            echo esc_attr($section['class']);
            ?>">
				<td colspan="2" class="forminp" style="display: flex; gap: 10px; flex-wrap: wrap; align-items: center;">
					<a style="margin-top: 10px;" href="<?php 
            echo esc_url("{$settings_link}&kec_action=simulate_webhook&nonce={$simulate_nonce}#klarna-payments-settings-kec_settings");
            ?>" class="button-link">
						<?php 
            esc_html_e('Test Webhook', 'klarna-express-checkout');
            ?>
					</a>
					<a style="margin-top: 10px;" href="<?php 
            echo esc_url("{$settings_link}&kec_action=delete_webhook&nonce={$delete_nonce}#klarna-payments-settings-kec_settings");
            ?>" class="button-link button-link-delete">
						<?php 
            esc_html_e('Remove Webhook', 'klarna-express-checkout');
            ?>
					</a>
				</td>
			</tr>
        ...
        ?>">
			<td colspan="2" class="forminp">
				<p><?php 
        esc_html_e('Create the necessary webhook in Klarna for the Express Checkout.', 'klarna-express-checkout');
        ?></p>
				<a style="margin-top: 10px;" href="<?php 
        echo esc_url("{$settings_link}&kec_action=create_webhook&nonce={$nonce}#klarna-payments-settings-kec_settings");
        ?>" class="button button-primary">
					<?php 
        esc_html_e('Create Webhook', 'klarna-express-checkout');
        ?>
				</a>
			</td>
		</tr>
    ...
    public static function maybe_show_messages()
    {
        ...
        $messages = \json_decode(\base64_decode(wp_unslash($_GET['kec_messages'])), \true);
    ...
    public static function maybe_show_errors()
    {
        ...
        $errors = \json_decode(\base64_decode(wp_unslash($_GET['kec_errors'])), \true);
        if (!\is_array($errors)) {
            return;
        }
