...
class PendingOrders
{
    ...
    public static function notification_listener($klarna_order_id = null, $data = null)
    {
        ...
        if ('ACCEPTED' === $klarna_order->fraud_status) {
            ...
            $order->update_meta_data('_wc_klarna_pending_to_cancelled', \true);
            $order->update_status('cancelled', 'Klarna order rejected.');
            $order->save();
            wc_mail(get_option('admin_email'), 'Klarna order rejected', \sprintf('Klarna has identified order %1$s, Klarna Reference %2$s as high risk and request that you do not ship this order. Please contact the Klarna Fraud Team to resolve.', $order->get_order_number(), $klarna_order->order_id));
        }
    }
    /**
    ...
    {
        $orders = wc_get_orders(array('meta_query' => array('meta_key' => '_wc_klarna_order_id', 'meta_value' => $klarna_order_id, 'compare' => '=')));
        $order = \reset($orders);
        if (empty($orders)) {
            return;
        }
    ...
    {
        return wc_get_order($order_id)->get_meta('_wc_klarna_order_id', \true);
    }
}
