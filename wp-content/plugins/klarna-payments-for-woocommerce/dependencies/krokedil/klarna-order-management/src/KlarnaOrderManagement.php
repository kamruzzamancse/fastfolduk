...
use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Settings;
use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Request\Get\RequestGetOrder;
use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Request\Post\RequestPostRefund;
use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Request\Post\RequestPostCapture;
use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Request\Patch\RequestPatchUpdate;
use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Request\Post\RequestPostCancel;
use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\MetaBox;
use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Ajax;
/**
...
class KlarnaOrderManagement
{
    ...
    public function init()
    {
                ...
                add_action('admin_notices', function () {
                    ?>
						<div class="notice notice-error">
								...
								<p><strong><?php 
                    esc_html_e('Klarna Order Management is now included in Klarna for WooCommerce.', 'klarna-order-management');
                    ?></strong></p>
								<p><?php 
                    esc_html_e('Starting with version 4.3.0, you no longer need the separate Klarna Order Management plugin – unless you are also using the Kustom Checkout plugin (formerly Klarna Checkout).', 'klarna-order-management');
                    ?></p>

								<p>
									<a href="https://docs.krokedil.com/klarna-for-woocommerce/get-started/order-management/#important-please-read" target="_blank">
										<?php 
                    esc_html_e('Read more about this change here.', 'klarna-order-management');
                    ?>
									</a>
								</p>
        ...
        if (\in_array('kom', $kp_unavailable_feature_ids, \true)) {
            return;
        }
    ...
    public function enqueue_admin()
    {
        ...
        wp_enqueue_script('kom-admin-js', plugin_dir_url(__FILE__) . 'assets/js/klarna-order-management.js', array('jquery'), '1.0.0', \true);
    ...
    public function cancel_klarna_order($order_id, $action = \false)
    {
        ...
        if (!isset($options['kom_auto_cancel']) || 'yes' === $options['kom_auto_cancel'] || $action) {
            ...
            if (!\in_array($order->get_payment_method(), array('klarna_payments', 'kco'), \true)) {
            ...
            if ($order->get_meta('_wc_klarna_pending_to_cancelled', \true)) {
    ...
    public function update_klarna_order_items($order_id, $items, $action = \false)
    {
        ...
        if (!isset($options['kom_auto_update']) || 'yes' === $options['kom_auto_update'] || $action) {
            ...
            if (!\in_array($order->get_status(), apply_filters('kom_allowed_update_statuses', array('on-hold')), \true)) {
            ...
            if (!\in_array($klarna_order->status, array('CANCELLED', 'CAPTURED', 'PART_CAPTURED'), \true)) {
                $request = new RequestPatchUpdate($this, array('request' => 'update_order_lines', 'order_id' => $order_id, 'klarna_order' => $klarna_order));
                $response = $request->request();
                if (!is_wp_error($response)) {
    ...
    public function capture_klarna_order($order_id, $action = \false)
    {
        ...
        if (!isset($options['kom_auto_capture']) || 'yes' === $options['kom_auto_capture'] || $action) {
            ...
            if ($order->get_meta('_wc_klarna_capture_id', \true)) {
            ...
            if (!$order->get_meta('_wc_klarna_order_id', \true) && !$order->get_transaction_id()) {
            ...
            if (\in_array($klarna_order->status, array('CAPTURED'), \true)) {
            ...
            if ('ACCEPTED' !== $klarna_order->fraud_status) {
                ...
                $request = new RequestPostCapture($this, array('request' => 'capture', 'order_id' => $order_id, 'klarna_order' => $klarna_order));
                $response = $request->request();
                if (!is_wp_error($response)) {
    ...
    public function refund_klarna_order($result, $order_id, $amount = null, $reason = '')
    {
        ...
        if (\in_array($klarna_order->status, array('CAPTURED', 'PART_CAPTURED'), \true)) {
            $request = new RequestPostRefund($this, array('order_id' => $order_id, 'refund_amount' => $amount, 'refund_reason' => $reason));
            $response = $request->request();
            if (!is_wp_error($response)) {
    ...
    {
        $request = new RequestGetOrder($this, array('order_id' => $order_id));
