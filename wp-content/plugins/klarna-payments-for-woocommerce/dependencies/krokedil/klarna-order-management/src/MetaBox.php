...
class MetaBox extends OrderMetabox
{
    ...
    {
        if (\in_array($post_type, array('woocommerce_page_wc-orders', 'shop_order'), \true)) {
            ...
            if (\in_array($order->get_payment_method(), array('kco', 'klarna_payments'), \true)) {
    ...
    public function metabox_content($post)
    {
        ...
        if (empty($order->get_date_paid()) && !\in_array($order->get_status(), array('on-hold'), \true)) {
        ...
        if (!empty($order->get_transaction_id()) || !empty($order->get_meta('_wc_klarna_order_id', \true))) {
    ...
    public function print_standard_content($klarna_order)
    {
        ...
        self::output_info(__('Klarna Environment', 'klarna-order-management'), apply_filters('kom_meta_environment', $environment));
        self::output_info(__('Klarna order status', 'klarna-order-management'), apply_filters('kom_meta_order_status', $klarna_order->status));
        self::output_info(__('Initial Payment method', 'klarna-order-management'), apply_filters('kom_meta_payment_method', $klarna_order->initial_payment_method->description));
        if (!empty($session_id)) {
            ...
            self::output_info(__('Scheduled actions', 'klarna-order-management'), '<a target="_blank" href="' . $link_url . '">' . $link_text . '</a>');
        }
    ...
    protected function output_actions_dropdown($order_id, $klarna_order)
    {
        ...
        $actions = array();
        $actions['capture'] = !isset($settings['kom_auto_capture']) || 'yes' === $settings['kom_auto_capture'] ? \false : \true;
        $actions['cancel'] = !isset($settings['kom_auto_cancel']) || 'yes' === $settings['kom_auto_cancel'] ? \false : \true;
        $actions['sync'] = !isset($settings['kom_auto_order_sync']) || 'yes' === $settings['kom_auto_order_sync'] ? \false : \true;
            ...
            ?>
				<li class="wide" id="kom-capture">
					<select class="kco_order_actions" name="kom_order_actions" id="kom_order_actions">
						<option value=""><?php 
            esc_attr_e('Choose an action...', 'woocommerce');
            ?></option>
						<?php 
    ...
    protected function get_advanced_section_content($order)
    {
        ...
        $enabled = 'enabled' === $om_status ? \true : \false;
        \ob_start();
    ...
    public function want_output_capture($order_id, $klarna_order, $actions)
    {
        ...
        if (!$actions['capture']) {
            return \false;
            // Capture is on auto, don't present action.
        }
        if (!empty($order->get_meta('_wc_klarna_capture_id'))) {
            return \false;
            // Already captured, can't capture again.
        }
        if (\in_array($klarna_order->status, array('CAPTURED', 'PART_CAPTURED', 'CANCELLED'), \true)) {
            return \false;
            // Klarna says it's captured (in whole, or in part) at some point. Can't capture again.
        }
    ...
    public function want_output_cancel($order_id, $klarna_order, $actions)
    {
        ...
        if (!$actions['cancel']) {
            return \false;
            // Cancel is on auto, don't present action.
        }
        if (!empty($order->get_meta('_wc_klarna_pending_to_cancelled', \true))) {
            return \false;
            // A cancellation is already pending, can't cancel again.
        }
        if (!\in_array($klarna_order->status, array('CAPTURED', 'PART_CAPTURED'), \true)) {
            return \false;
            // Can only cancel orders that are captured.
        }
        return \true;
    }
    /**
    ...
    public function process_kom_actions($post_id, $post)
    {
        ...
        if ('kom_capture' === $kom_action) {
            $this->order_management->capture_klarna_order($post_id, \true);
        }
        // Cancel order.
        if ('kom_cancel' === $kom_action) {
            $this->order_management->cancel_klarna_order($post_id, \true);
        }
        // Sync order.
    ...
    {
        if (!\class_exists('KrokedilKlarnaPaymentsDeps\\ActionScheduler')) {
            return array();
        }
        $actions = as_get_scheduled_actions(array('args' => array($order_id), 'per_page' => -1, 'status' => ''));
        $action_list = array();
        foreach ($actions as $action) {
            $action_list[] = array('name' => $action->get_hook(), 'status' => $action->get_status(), 'scheduled' => $action->get_scheduled_date()->format('Y-m-d H:i:s'));
        }
        return $action_list;
    }
    /**
    ...
    public function maybe_localize_script($handle)
    {
        if ('kom-admin-js' === $handle) {
            $localize_data = array('ajax' => array('setOrderSync' => array('url' => admin_url('admin-ajax.php'), 'action' => 'woocommerce_kom_wc_set_order_sync', 'nonce' => wp_create_nonce('kom_wc_set_order_sync'))), 'orderId' => $this->get_id());
