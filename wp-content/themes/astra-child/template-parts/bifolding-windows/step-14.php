 ...
 * Step 14: Your Order Summary - COMPLETE VERSION FOR WINDOWS
 *
 * @package Astra Child
 */
?>
...
<div class="wizard-step" data-step="14">
    <div class="step-container summary-container">
        ...
        <div class="summary-header">
            <h1 class="summary-title">Aluminium Bifolding Window</h1>
        ...
        <div class="summary-section">
            <div class="summary-grid">
                ...
                <div class="summary-row"><span class="summary-label">Frame Colour (Outside):</span><span class="summary-value" id="summary-outside-colour">—</span></div>
                <div class="summary-row"><span class="summary-label">Frame Colour (Inside):</span><span class="summary-value" id="summary-inside-colour">—</span></div>
...
<style>
/* ================================
   Order Summary Step Styles for Window Builder
   ================================ */
...
<script>
/**
 * Step 14 - Order Summary for Window Builder
 */
jQuery(document).ready(function($) {
    ...
    /**
     * Calculate total price for windows
     */
    function calculateTotalPrice() {
        ...
        if (installType) {
            if (installType === 'install_existing') {
                total += 299;
                if (isDev()) console.log('Installation: Existing Opening - £299');
            } else if (installType === 'install_new_build') {
                total += 499;
                if (isDev()) console.log('Installation: New Build Opening - £499');
    ...
    /**
     * Get installation display text for windows
     */
    function getInstallationDisplay() {
        ...
        if (val === 'install_existing') {
            ...
            } else if (deliveryPrice === 0 && $('#window_delivery_bespoke').val() !== '1') {
                displayText += ' (FREE)';
            }
        }
    ...
    /**
     * Get delivery information for windows
     */
    function getDeliveryInfo() {
        ...
        if (window.windowDeliveryData) {
            if (window.windowDeliveryData.bespoke) {
                deliveryInfo.display = 'Bespoke (call for quote)';
                deliveryInfo.isBespoke = true;
            } else {
                const price = parseFloat(window.windowDeliveryData.price) || 0;
                deliveryInfo.price = price;
                const priceText = price === 0 ? 'FREE' : '£' + price.toFixed(2);
                const zone = window.windowDeliveryData.zone || '';
                const distance = window.windowDeliveryData.distance ? parseFloat(window.windowDeliveryData.distance).toFixed(1) + ' miles' : '';
            ...
            const price = parseFloat($('#window_delivery_price').val()) || 0;
            const isBespoke = $('#window_delivery_bespoke').val() === '1';
            const zone = $('#window_delivery_zone').val() || '';
            const distance = $('#window_delivery_distance').val() ? parseFloat($('#window_delivery_distance').val()).toFixed(1) + ' miles' : '';
    ...
    /**
     * Submit builder form for windows
     */
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    window.location.href = window.windowBuilderData?.cart_url || '';
                } else {
    ...
    /**
     * Populate Step 14 with all values for windows
     */
    window.populateStep14 = function() {
        ...
        let panelText = '';
        if (panelValue) {
            panelText = $('input[name="window_panel_layout"]:checked').closest('.panel-option-card').find('.option-name').text().trim();
            if (!panelText) {
                const panelMap = {
        ...
        const handleColour = $('input[name="window_handle_colour"]:checked').val();
        const handleMap = {
            'white': 'White',
            'chrome': 'Chrome',
            'black': 'Black',
        ...
        // === STEP 7: Glass ===
        const glassValue = $('input[name="glass_type"]:checked').val();
        let glassText = '—';
        ...
        // === STEP 10: Postcode ===
        const postcode = $('#window_postcode').val() || '—';
        ...
        const accessIssues = $('input[name="window_access_issues"]:checked').val();
        let accessText = 'No';
        
        if (accessIssues === 'yes_access') {
            const accessDesc = $('#window_access_description').val();
        ...
        if (isDev()) {
            console.log('Step 14 populated for Window - Total: £' + totalPriceFormatted);
    ...
    if (isDev()) {
        console.log('Step 14 script loaded successfully for Window Builder');
    }
