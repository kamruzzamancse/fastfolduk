...
class OneStepCheckout
{
    ...
    public static function maybe_redirect_kec_one_step_checkout()
    {
        ...
        $args = array(
            ...
            // Only look for orders created via KEC.
            'date_created' => '>' . (\time() - DAY_IN_SECONDS * 2),
        );
    ...
    public static function get_redirect_url_for_order($order, $kec_unique_id)
    {
        ...
        // Try a maximum of 20 times to get the redirect url for the order.
        $sleep_time = apply_filters('kec_one_step_redirect_wait_sleep_time_mu', 5 * 100000);
        // Sleep for 0.5 seconds between attempts.
        $attempt = 0;
    ...
    public static function get_initiate_body()
    {
        ...
        return array('collectCustomerProfile' => array('profile:name', 'profile:email', 'profile:phone', 'profile:locale', 'profile:billing_address', 'profile:country'), 'shippingConfig' => array('mode' => 'EDITABLE'), 'paymentRequestReference' => $unique_id, 'customerInteractionConfig' => array('returnUrl' => add_query_arg(array('kec-one-step' => $unique_id), home_url())), 'amount' => self::format_price(WC()->cart->get_cart_contents_total() + WC()->cart->get_cart_contents_tax()), 'currency' => get_woocommerce_currency(), 'supplementaryPurchaseData' => array('lineItems' => self::get_cart_items()));
    }
    /**
    ...
    public static function get_changed_shipping_option_response($selected_option)
    {
        ...
        if (!empty($selected_shipping_method)) {
            $line_items[] = array('name' => $selected_shipping_method->get_label(), 'shippingReference' => $selected_shipping_method->get_id(), 'quantity' => 1, 'totalAmount' => self::format_price($selected_shipping_method->get_cost() + $selected_shipping_method->get_shipping_tax()), 'totalTaxAmount' => self::format_price(WC()->cart->get_shipping_tax()));
        ...
        return array('amount' => self::format_price(WC()->cart->get_cart_contents_total() + WC()->cart->get_cart_contents_tax() + $selected_shipping_method->get_cost() + $selected_shipping_method->get_shipping_tax()), 'lineItems' => $line_items);
    }
    /**
    ...
    public static function get_shipping_options($shipping_packages)
    {
        ...
        foreach ($shipping_packages as $package) {
            ...
            foreach ($package['rates'] as $rate) {
                ...
                $shipping_options[] = array('shippingOptionReference' => $rate->get_id(), 'amount' => self::format_price($rate->get_cost() + $rate->get_shipping_tax()), 'displayName' => $rate->get_label());
            }
        }
    ...
    private static function get_wc_order()
    {
        ...
        $order = wc_create_order(array('status' => 'pending', 'customer_id' => get_current_user_id(), 'created_via' => 'klarna_express_checkout'));
        if (!$order || is_wp_error($order)) {
            return \false;
        }
        return $order;
    ...
    private static function set_order_item_products(&$order)
    {
        foreach (WC()->cart->get_cart() as $cart_item) {
            ...
            $order->add_product($product, $cart_item['quantity'], array('totals' => array('subtotal' => $cart_item['line_subtotal'], 'subtotal_tax' => $cart_item['line_subtotal_tax'], 'total' => $cart_item['line_total'], 'tax' => $cart_item['line_tax'])));
        }
    }
    /**
