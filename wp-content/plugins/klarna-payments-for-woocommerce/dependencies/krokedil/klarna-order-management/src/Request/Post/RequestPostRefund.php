...
namespace KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Request\Post;

use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Request\RequestPost;
use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\OrderLines;
use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Utility;
if (!\defined('ABSPATH')) {
    exit;
}
/**
...
class RequestPostRefund extends RequestPost
{
    ...
    {
        $data = array('refunded_amount' => \round($this->refund_amount * 100), 'description' => $this->refund_reason);
        ...
        if (!empty($refund_id)) {
            ...
            if ($refunded_items) {
                ...
                foreach ($refunded_items as $item) {
                    ...
                    $item_data = array('type' => $type, 'reference' => $reference, 'name' => $name, 'quantity' => $quantity, 'unit_price' => $unit_price, 'tax_rate' => $order_line_tax_rate, 'total_amount' => $total, 'total_discount_amount' => $total_discount, 'total_tax_amount' => $refund_tax_amount);
                    ...
                    if (!($separate_sales_tax && '0' == $refund_price_amount)) {
                        // phpcs:ignore WordPress.PHP.StrictComparisons -- Can be float *or* integer, so non-strict is required.
                        $data[] = $item_data;
                    }
                }
            }
            ...
            if ($refunded_shipping) {
                ...
                foreach ($refunded_shipping_items as $shipping_item) {
                    ...
                    $total = \round($quantity * $unit_price);
                    $shipping_data = array('type' => $type, 'reference' => $reference, 'name' => $name, 'quantity' => $quantity, 'unit_price' => $unit_price, 'tax_rate' => $order_shipping_tax_rate, 'total_amount' => $total, 'total_discount_amount' => $total_discount, 'total_tax_amount' => $refund_tax_amount);
                    // Do not add order lines if separate sales tax and no refund amount entered.
                    if (!($separate_sales_tax && '0' == $refund_price_amount)) {
                        // phpcs:ignore WordPress.PHP.StrictComparisons -- Can be float *or* integer, so non-strict is required.
                        $data[] = $shipping_data;
