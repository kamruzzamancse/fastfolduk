...
namespace KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Request;

use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\KlarnaOrderManagement;
use KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement\Logger;
if (!\defined('ABSPATH')) {
    exit;
}
/**
...
abstract class Request
{
    ...
    {
        $order = wc_get_order($this->order_id);
        return !empty($order->get_transaction_id()) ? $order->get_transaction_id() : $order->get_meta('_wc_klarna_order_id', \true);
    }
    /**
    ...
    {
        $order = wc_get_order($this->order_id);
        $country = $order->get_meta('_wc_klarna_country', \true);
     ...
     * @return string
     */
    protected abstract function get_request_url();
    /**
     * Make the request.
     *
    ...
    protected function get_request_headers()
    {
        ...
        return array('Authorization' => $auth, 'Content-Type' => 'application/json');
    }
    /**
    ...
    {
        $order = wc_get_order($this->order_id);
        $component = $order->get_meta("_wc_klarna_{$component_name}", \true);
        if (!empty($component)) {
            return \iconv(\mb_detect_encoding($component, \mb_detect_order(), \true), 'UTF-8', $component);
        }
        $variant = $this->get_klarna_variant();
        if (empty($variant)) {
            return '';
    ...
    protected function process_response($response, $request_args, $request_url)
    {
        ...
        if ($response_code < 200 || $response_code >= 300) {
            // Anything not in the 200 range is an error.
    ...
    public function get_request_args()
    {
        ...
        $args = array('headers' => $headers, 'user-agent' => $this->get_user_agent(), 'method' => $this->method, 'timeout' => apply_filters('kom_request_timeout', 10));
        $body = $this->get_body();
        if (!empty($body)) {
