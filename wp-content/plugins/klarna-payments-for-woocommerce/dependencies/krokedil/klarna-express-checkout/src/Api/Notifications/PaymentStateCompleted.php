...
namespace KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaExpressCheckout\Api\Notifications;
...
class PaymentStateCompleted extends Handler
{
    ...
    public function handle_notification($payload)
    {
        ...
        if ('COMPLETED' !== $payload['state']) {
            return;
            // Maybe throw an error or log this incident.
        }
        ...
        if ($ap_partner_integration) {
            $redirect_url = $ap_partner_integration->process_order_state($order, $interoperability_token, [], $payload['state'], $payload);
        } else {
            do_action('kec_process_order', $order, $interoperability_token, [], $payload['state'], $payload);
        }
    ...
    protected function set_address(&$order, $payload)
    {
        ...
        $this->set_address_field($order, $billing_customer['given_name'] ?? '', 'first_name', 'billing')->set_address_field($order, $billing_customer['family_name'] ?? '', 'last_name', 'billing')->set_address_field($order, $billing_customer['email'] ?? '', 'email', 'billing')->set_address_field($order, $billing_customer['phone'] ?? '', 'phone', 'billing')->set_address_field($order, $billing_address['street_address'] ?? '', 'address_1', 'billing')->set_address_field($order, $billing_address['street_address2'] ?? '', 'address_2', 'billing')->set_address_field($order, $billing_address['postal_code'] ?? '', 'postcode', 'billing')->set_address_field($order, $billing_address['city'] ?? '', 'city', 'billing')->set_address_field($order, $billing_address['region'] ?? '', 'state', 'billing')->set_address_field($order, $billing_address['country'] ?? '', 'country', 'billing')->set_address_field($order, $shipping_customer['given_name'] ?? '', 'first_name', 'shipping')->set_address_field($order, $shipping_customer['family_name'] ?? '', 'last_name', 'shipping')->set_address_field($order, $shipping_customer['email'] ?? '', 'email', 'shipping')->set_address_field($order, $shipping_customer['phone'] ?? '', 'phone', 'shipping')->set_address_field($order, $shipping_address['street_address'] ?? '', 'address_1', 'shipping')->set_address_field($order, $shipping_address['street_address2'] ?? '', 'address_2', 'shipping')->set_address_field($order, $shipping_address['postal_code'] ?? '', 'postcode', 'shipping')->set_address_field($order, $shipping_address['city'] ?? '', 'city', 'shipping')->set_address_field($order, $shipping_address['region'] ?? '', 'state', 'shipping')->set_address_field($order, $shipping_address['country'] ?? '', 'country', 'shipping');
        return $this;
    }
    /**
    ...
    protected function set_address_field(&$order, $value, $field, $address_type)
    {
        if (!empty($value)) {
            ...
            if (\method_exists($order, $method)) {
                // Ensure the method exists before we call it.
                $order->{$method}($value);
            } else {
                // Fallback to using meta data if the method doesn't exist.
