...
namespace KrokedilKlarnaPaymentsDeps\Krokedil\KlarnaOrderManagement;
...
class OrderLines
{
    ...
    public function __construct($order_id, $request_type = '')
    {
        ...
        if ('US' === $shop_country) {
            $this->separate_sales_tax = \true;
        }
        $this->klarna_country = \strtoupper($this->order->get_meta('_wc_klarna_country', \true));
    }
    /**
    ...
    public function order_lines()
    {
        ...
        return array('order_lines' => $this->order_lines, 'order_amount' => $this->order_amount, 'order_tax_amount' => $this->order_tax_amount);
    }
    /**
    ...
    public function process_order_line_items()
    {
        ...
        foreach ($order->get_items('pw_gift_card') as $gift_card) {
            ...
            $order_item = array('type' => 'gift_card', 'reference' => $gift_card_sku, 'name' => __('Gift card', 'pw-woocommerce-gift-cards') . ' ' . $code, 'quantity' => 1, 'tax_rate' => 0, 'total_discount_amount' => 0, 'total_tax_amount' => 0, 'unit_price' => $gift_card_amount, 'total_amount' => $gift_card_amount);
            $this->order_lines[] = $order_item;
        }
        $added_surcharge = \json_decode($this->order->get_meta('_kco_added_surcharge', \true), \true);
    ...
    {
        $reference = \json_decode($this->order->get_meta('_kco_kss_data', \true), \true);
        return array('reference' => isset($reference['id']) ? $reference['id'] : $this->get_item_reference($order_item), 'type' => 'shipping_fee', 'name' => $this->get_item_name($order_item), 'quantity' => 1, 'unit_price' => $this->get_item_unit_price($order_item), 'tax_rate' => $this->get_item_tax_rate($order, $order_item), 'total_amount' => $this->get_item_total_amount($order_item), 'total_discount_amount' => $this->get_item_discount_amount($order_item), 'total_tax_amount' => $this->get_item_tax_amount($order_item));
    }
    /**
    ...
    {
        return array('reference' => $this->get_item_reference($order_item), 'type' => 'surcharge', 'name' => $this->get_item_name($order_item), 'quantity' => 1, 'unit_price' => $this->get_item_unit_price($order_item), 'tax_rate' => $this->get_item_tax_rate($order, $order_item), 'total_amount' => $this->get_item_total_amount($order_item), 'total_discount_amount' => $this->get_item_discount_amount($order_item), 'total_tax_amount' => $this->get_item_tax_amount($order_item));
    }
    /**
    ...
    {
        $klarna_item = array('reference' => $this->get_item_reference($order_item), 'type' => 'discount', 'name' => $this->get_item_name($order_item), 'quantity' => 1, 'unit_price' => $this->get_item_unit_price($order_item), 'tax_rate' => $this->get_item_tax_rate($order, $order_item), 'total_amount' => $this->get_item_total_amount($order_item), 'total_discount_amount' => 0, 'total_tax_amount' => $this->get_item_tax_amount($order_item));
        $coupon = new \WC_Coupon($order_item->get_name());
    ...
    public function process_sales_tax()
    {
        if ($this->separate_sales_tax) {
            ...
            $sales_tax = array('type' => 'sales_tax', 'reference' => __('Sales Tax', 'klarna-order-management'), 'name' => __('Sales Tax', 'klarna-order-management'), 'quantity' => 1, 'unit_price' => $sales_tax_amount, 'tax_rate' => 0, 'total_amount' => $sales_tax_amount, 'total_discount_amount' => 0, 'total_tax_amount' => 0);
    ...
    public function get_item_unit_price($order_line_item)
    {
        ...
        $item_price = \number_format($item_price * 100, 0, '', '') / $item_quantity;
    ...
    public function get_item_tax_amount($order_line_item)
    {
        if ($this->separate_sales_tax) {
            $item_tax_amount = 00;
        } elseif (\in_array($order_line_item->get_type(), array('line_item', 'fee', 'shipping'), \true)) {
