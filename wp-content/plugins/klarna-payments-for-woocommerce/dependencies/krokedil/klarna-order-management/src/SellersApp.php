...
class SellersApp
{
    ...
    public function process_order_creation($post_id, $post, $update)
    {
        ...
        if ($update && !empty($order->get_transaction_id()) && \in_array($order->get_payment_method(), array('kco', 'klarna_payments'), \true)) {
            // Set post metas.
    ...
    public static function populate_klarna_order($post_id, $klarna_order)
    {
        ...
        $process_order_lines = \true;
        if (\true === $process_order_lines) {
            self::process_order_lines($klarna_order, $order);
            $order->set_date_paid(\time());
        ...
        foreach ($klarna_order->order_lines as $cart_item) {
            ...
            if ('physical' === $cart_item->type || 'digital' === $cart_item->type) {
                ...
                try {
                    ...
                    $args = array('name' => $product->get_name(), 'tax_class' => $product->get_tax_class(), 'product_id' => $product->is_type('variation') ? $product->get_parent_id() : $product->get_id(), 'variation_id' => $product->is_type('variation') ? $product->get_id() : 0, 'variation' => $product->is_type('variation') ? $product->get_attributes() : array(), 'subtotal' => ($cart_item->total_amount - $cart_item->total_tax_amount) / 100, 'total' => ($cart_item->total_amount - $cart_item->total_tax_amount) / 100, 'quantity' => $cart_item->quantity);
            ...
            if ('surcharge' === $cart_item->type) {
                ...
                try {
                    $args = array('name' => $cart_item->name, 'tax_class' => $tax_class, 'subtotal' => ($cart_item->total_amount - $cart_item->total_tax_amount) / 100, 'total' => ($cart_item->total_amount - $cart_item->total_tax_amount) / 100, 'quantity' => $cart_item->quantity);
